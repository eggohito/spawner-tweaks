...
#   Randomize the `Delay` NBT based from the copied Spawner data's `MinSpawnDelay` and `MaxSpawnDelay` NBTs
...
execute if score shuffle_spawn_data spawner-tweaks matches 0 if data storage spawner:tweaks temp.block.SpawnData run data modify block ~ ~ ~ SpawnData set from storage spawner:tweaks temp.block.SpawnData

execute if score shuffle_spawn_data spawner-tweaks matches 0 unless data storage spawner:tweaks temp.block.SpawnData run function spawner-tweaks:impl/spawner/block/choose/spawn_potentials
