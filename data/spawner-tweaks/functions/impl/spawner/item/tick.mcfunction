...
execute if score #selected.placeholder_item.age spawner-tweaks matches 1 if predicate spawner-tweaks:item/dispensed as @e[type = minecraft:minecart, predicate = spawner-tweaks:minecart/vehicle/no_passengers, distance = ..0.8] at @s run function spawner-tweaks:impl/spawner/minecart/summon
